...
@Component({ selector: 'app-root', standalone: true, imports: [CommonModule, FormsModule, ArchiveComponent], templateUrl: './app.component.html', styleUrl: './app.component.css' })
export class AppComponent implements OnInit {
  ...
  emptyExpense() { return { property_id: 0, category: 'Repair', amount: 0, expense_date: new Date().toISOString().slice(0, 10), paid_to: '', payment_method: 'Bank transfer', reference: '', notes: '' }; }
  ...
  removeExpenseRecord(exp: PropertyExpense) {
    if (confirm(`Delete expense record #${exp.id} (${exp.category} - ${this.currency(exp.amount)})?`)) {
      this.api.deleteExpense('expenses', exp.id).subscribe({
  ...
  emptyRefund() { return { tenancy_id: 0, amount: 0, refunded_on: new Date().toISOString().slice(0, 10), payment_method: 'Bank transfer', reference: '', deduction_amount: 0, deduction_reason: '', is_final_settlement: true, notes: '' }; }
  openTenant(t?: Tenant) { this.tenantForm = t ? {...t} : {id: 0,name:'',phone:'',notes:''}; this.modal='tenant'; }
  openTenancy(t?: Tenancy) { this.tenancyForm = t ? {...t,business_name:t.business_name || '',start_date:t.start_date || '',end_date:t.end_date || ''} : {id:0,property_id:0,tenant_id:0,business_name:'',start_date:'',end_date:'',deposit:0,initial_rent:null,notes:''}; this.modal='tenancy'; }
  openRate(t: Tenancy, r?: RentRate) { this.rateForm = r ? {...r} : {id:0,tenancy_id:t.id,effective_month:'',amount:t.initial_rent ?? 0}; this.modal='rate'; }
  openRefund(t: Tenancy, r?: DepositRefund) {
    this.selectedTenancyForRefund = t;
    this.editingRefund = r?.id || 0;
    const remaining = (t.deposit_balance !== undefined ? t.deposit_balance : t.deposit) || 0;
    this.refundForm = r
      ? {
          tenancy_id: t.id,
          amount: r.amount,
          refunded_on: r.refunded_on || new Date().toISOString().slice(0, 10),
  ...
  setDeductionReason(reason: string) {
    this.refundForm.deduction_reason = reason;
  }
  autoFillRemainingDeduction() {
    if (!this.selectedTenancyForRefund) return;
    const initialDeposit = this.selectedTenancyForRefund.deposit || 0;
    const otherRefunds = (this.selectedTenancyForRefund.refunds || []).filter(r => r.id !== this.editingRefund);
    const otherTotal = otherRefunds.reduce((sum, r) => sum + r.amount + (r.deduction_amount || 0), 0);
    const available = Math.max(initialDeposit - otherTotal, 0);
    const refundAmt = Number(this.refundForm.amount) || 0;
    this.refundForm.deduction_amount = Math.max(available - refundAmt, 0);
    if (!this.refundForm.deduction_reason) {
      this.refundForm.deduction_reason = 'Whitewashing & painting charges';
    }
  }
  setFullDeduction() {
    if (!this.selectedTenancyForRefund) return;
    ...
    this.refundForm.deduction_amount = available;
    this.refundForm.is_final_settlement = true;
    if (!this.refundForm.deduction_reason) {
      this.refundForm.deduction_reason = 'Whitewashing, repairs and painting charges';
    }
  }
  ...
  openTransfer(t: Tenancy) {
    ...
    // Find current rent for this tenancy
  ...
  tenancyRates(id: number) { return this.rates.filter(r=>r.tenancy_id===id); }
  get paymentTenancies() { return this.tenancies.filter(t=>t.property_id===this.paymentForm.property_id && (!t.start_date || t.start_date.slice(0,7)<=this.paymentForm.rental_month) && (!t.end_date || t.end_date.slice(0,7)>=this.paymentForm.rental_month)); }
  get paymentNeedsTenancy() { return this.tenancies.some(t=>t.property_id===this.paymentForm.property_id); }
  ...
  saveTenant() { this.api.saveTenancyRecord('tenants',this.tenantForm.id,this.tenantForm).subscribe({next:()=>this.done('Tenant saved'),error:e=>this.showError(e)}); }
  saveTenancy() { this.api.saveTenancyRecord('tenancies',this.tenancyForm.id,{...this.tenancyForm,start_date:this.tenancyForm.start_date || null,end_date:this.tenancyForm.end_date || null,initial_rent:this.tenancyForm.start_date ? this.tenancyForm.initial_rent : null}).subscribe({next:()=>this.done('Tenancy saved'),error:e=>this.showError(e)}); }
  saveRate() { this.api.saveTenancyRecord('rent-rates',this.rateForm.id,this.rateForm).subscribe({next:()=>this.done('Rent rate saved'),error:e=>this.showError(e)}); }
  saveRefund() { const req = this.editingRefund ? this.api.updateDepositRefund(this.editingRefund, this.refundForm) : this.api.addDepositRefund(this.refundForm); req.subscribe({ next: () => this.done('Deposit refund saved'), error: e => this.showError(e) }); }
  removeRefund(r: DepositRefund) { if (confirm('Delete this deposit refund record?')) this.api.deleteDepositRefund(r.id).subscribe({ next: () => this.done('Deposit refund deleted'), error: e => this.showError(e) }); }
  history: EditHistory[] = [];
  historyFilter = '';
  editingGroup = 0;
  editingPayment = 0;
  ...
  get visibleHistory() { return this.history.filter(h => !this.historyFilter || h.entity === this.historyFilter); }
  entityName(entity: string) { return ({tenants:'Tenant',tenancies:'Tenancy',rent_rates:'Rent rate',deposit_refunds:'Deposit refund',property_expenses:'Property expense',properties: 'Property', property_groups: 'Group', rent_payments: 'Rent payment', group_taxes: 'Tax payment', electricity_bills: 'Electricity bill'} as Record<string,string>)[entity] || entity; }
  fieldName(field: string) { return field.replace(/_/g, ' '); }
  historyValue(value: unknown) { return value === null || value === '' ? '—' : String(value); }
  openGroup(group?: PropertyGroup) { this.editingGroup = group?.id || 0; this.groupForm = group ? {name: group.name, kind: group.kind} : {name: '', kind: 'Apartment'}; this.modal = 'group'; }
  editPayment(payment: Payment) { this.editingPayment = payment.id; this.paymentForm = {...payment}; this.modal = 'payment'; }
  editTax(tax: GroupTax) { this.editingTax = tax.id; this.taxForm = {...tax}; this.modal = 'tax'; }
  editElectricity(bill: ElectricityBill) { this.editingElectricity = bill.id; this.electricityForm = {...bill}; this.modal = 'electricity'; }
  ...
  get ownerProperties() { return this.properties.filter(p => p.electricity_payer === 'Owner' || (this.editingElectricity > 0 && p.id === this.electricity.find(b => b.id === this.editingElectricity)?.property_id)); }
  ...
  get visibleTaxes() {
    return this.taxes.filter(t => {
      const matchesGroup = !this.taxGroupFilter || t.group_id === +this.taxGroupFilter;
      const matchesType = !this.taxTypeFilter || t.tax_type === this.taxTypeFilter;
      const matchesFrom = !this.taxFromDate || (t.paid_on && t.paid_on >= this.taxFromDate);
      const matchesTo = !this.taxToDate || (t.paid_on && t.paid_on <= this.taxToDate);
      return matchesGroup && matchesType && matchesFrom && matchesTo;
    });
  }
  get visibleTaxesTotal(): number { return this.visibleTaxes.reduce((sum, t) => sum + (t.amount || 0), 0); }
  get visiblePropertyTaxTotal(): number { return this.visibleTaxes.filter(t => t.tax_type === 'Property tax').reduce((sum, t) => sum + (t.amount || 0), 0); }
  get visibleWaterTaxTotal(): number { return this.visibleTaxes.filter(t => t.tax_type === 'Water tax').reduce((sum, t) => sum + (t.amount || 0), 0); }
  openTax(groupId = 0) { this.editingTax = 0; this.taxForm = this.emptyTax(); this.taxForm.group_id = groupId; this.modal = 'tax'; }
  openElectricity() { this.editingElectricity = 0; this.electricityForm = this.emptyElectricity(); this.modal = 'electricity'; }
  openCollection(bill: ElectricityBill) { this.collectionForm = { id: bill.id, collected: bill.collected, amount: bill.amount }; this.modal = 'collection'; }
  ...
  removeGroup(group: PropertyGroup) {
    ...
    this.api.deleteGroup(group.id).subscribe({
      next: () => {
        if (this.groupFilter === String(group.id)) this.groupFilter = '';
        if (this.taxGroupFilter === String(group.id)) this.taxGroupFilter = '';
        this.done('Group deleted; properties and payments preserved');
      },
      error: e => this.showError(e)
    });
  }
  saveGroup() { (this.editingGroup ? this.api.updateRecord('groups', this.editingGroup, this.groupForm) : this.api.addGroup(this.groupForm)).subscribe({ next: () => { this.groupForm = { name: '', kind: 'Apartment' }; this.done('Group saved'); }, error: e => this.showError(e) }); }
  saveTax() { (this.editingTax ? this.api.updateRecord('taxes', this.editingTax, this.taxForm) : this.api.addTax(this.taxForm)).subscribe({ next: () => this.done('Tax payment saved'), error: e => this.showError(e) }); }
  saveElectricity() { (this.editingElectricity ? this.api.updateRecord('electricity', this.editingElectricity, this.electricityForm) : this.api.addElectricity(this.electricityForm)).subscribe({ next: () => this.done('Electricity bill saved'), error: e => this.showError(e) }); }
  ...
  removeExpense(type: 'taxes' | 'electricity', id: number) { if (confirm('Delete this record?')) this.api.deleteExpense(type, id).subscribe({ next: () => this.done('Record deleted'), error: e => this.showError(e) }); }
  showError(e: any) { this.error = typeof e.error?.detail === 'string' ? e.error.detail : 'Please check the entered details and try again.'; }
  ...
  get propertyHasTenancyHistory() { return this.properties.find(p=>p.id===this.propertyForm.id)?.has_tenancy_history || false; }
  propertyTypeIcon(type?: string) {
    switch (type) {
      case 'Shop': return '🏪';
  ...
  billingCycleDescription(rentalMonth?: string, paidOn?: string): string {
    ...
    const paidMonthName = paidDate.toLocaleDateString('en-US', { month: 'short', year: 'numeric' });
  ...
  get filteredTenancies() {
    ...
    return this.tenancies.filter(t => {
      const matchesGroup = !this.tenancyGroupFilter ||
        (this.tenancyGroupFilter === 'ungrouped' ? !t.group_id : t.group_id === +this.tenancyGroupFilter);
      const matchesSearch = !q || [t.property_name, t.tenant_name, t.business_name, t.group_name].some(v => v?.toLowerCase().includes(q));
  ...
  get filteredPayments() {
    ...
    return this.payments.filter(p => {
      const matchesSearch = !q || [p.property_name, p.tenant_name, p.business_name, p.reference, p.group_name].some(v => v?.toLowerCase().includes(q));
      const matchesProperty = !this.paymentPropertyFilter || p.property_id === +this.paymentPropertyFilter;
      const matchesTenant = !this.paymentTenantFilter || p.tenant_name?.toLowerCase() === this.paymentTenantFilter.toLowerCase();
      const matchesFrom = !this.paymentFromMonth || p.rental_month >= this.paymentFromMonth;
      const matchesTo = !this.paymentToMonth || p.rental_month <= this.paymentToMonth;
      return matchesSearch && matchesProperty && matchesTenant && matchesFrom && matchesTo;
    });
  }
  get filteredPaymentsTotal(): number { return this.filteredPayments.reduce((sum, p) => sum + (p.amount || 0), 0); }
  tenantTotalPaid(tenantName: string): number { return this.payments.filter(p => p.tenant_name?.toLowerCase() === tenantName.toLowerCase()).reduce((sum, p) => sum + (p.amount || 0), 0); }
  propertyTotalPaid(propertyId: number): number { return this.payments.filter(p => p.property_id === propertyId).reduce((sum, p) => sum + (p.amount || 0), 0); }
  get filteredExpenses() {
    const q = this.expenseSearch.toLowerCase();
    return this.expenses.filter(e => {
      const matchesProperty = !this.expensePropertyFilter || e.property_id === +this.expensePropertyFilter;
      const matchesGroup = !this.expenseGroupFilter || (this.expenseGroupFilter === 'ungrouped' ? !e.group_id : e.group_id === +this.expenseGroupFilter);
      const matchesCategory = !this.expenseCategoryFilter || e.category === this.expenseCategoryFilter;
      const matchesFrom = !this.expenseFromDate || (e.expense_date && e.expense_date >= this.expenseFromDate);
      const matchesTo = !this.expenseToDate || (e.expense_date && e.expense_date <= this.expenseToDate);
      const matchesSearch = !q || [e.property_name, e.category, e.paid_to, e.reference, e.notes, e.group_name].some(v => v?.toLowerCase().includes(q));
      return matchesProperty && matchesGroup && matchesCategory && matchesFrom && matchesTo && matchesSearch;
    });
  }
  get filteredExpensesTotal(): number { return this.filteredExpenses.reduce((sum, e) => sum + (e.amount || 0), 0); }
  get expensesCategoryBreakdown(): Array<{ category: string; total: number; count: number }> {
